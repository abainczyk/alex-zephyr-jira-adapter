...
export const projectMappingsStore = {
  ...
  mutations: {
    ...
    remove(state, mappingId) {
      state.projectMappings = state.projectMappings.filter(pm => pm.id !== mappingId);
    },
  ...
  actions: {
    ...
    remove({commit}, mappingId) {
      return projectMappingApi.delete(mappingId)
        .then(res => {
          commit('remove', mappingId);
          return res;
        });
    },
